...
export default function Portfolio() {
  ...
  return (
    <div
      className={`min-h-screen ${t.bg} ${t.text} font-sans transition-colors duration-300`}
    >
      {/* Theme toggle */}
      <div className="fixed top-4 right-4 z-50">
        <div
          className={`flex items-center gap-1 ${t.surface} border ${t.border} rounded-full p-1 shadow-lg`}
        >
          <button
      ...
      <div className={`${t.surface} border-b ${t.border}`}>
        <div className="max-w-4xl mx-auto px-8 py-16">
          <div className="flex items-start justify-between">
            ...
            <div
              className={`hidden md:block w-24 h-24 rounded-2xl ${t.accentBlock} opacity-20`}
            />
          </div>
        </div>
      </div>
              ...
              {experiences.map((exp) => (
                <div key={exp._id} className="flex gap-6">
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-3 h-3 rounded-full ${t.accentBg} mt-1.5 flex-shrink-0`}
                    />
                  ...
                  <div className="flex-1 pb-6">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className={`${t.accent} text-sm mt-0.5`}>
                          {exp.company}
                        </p>
                      </div>
                      <p
                        className={`text-xs ${t.textSubtle} flex-shrink-0 mt-1`}
                      >
                        {exp.startDate} —{" "}
                    ...
                    {exp.description && (
                      <p
                        className={`${t.textMuted} text-sm mt-2 leading-relaxed`}
                      >
                        {exp.description}
                      </p>
                    )}
          ...
          <section id="education">
            ...
            <div className="flex flex-col gap-6 mt-4">
              {education.map((edu) => (
                <div
                  key={edu._id}
                  className={`rounded-2xl p-6 border ${t.card} shadow-sm`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className={`font-semibold ${t.text}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              {displayed.map((project) => (
                ...
                >
                  ...
                  {project.description && (
                    <p
                      className={`text-sm ${t.textMuted} leading-relaxed line-clamp-3`}
                    >
                      {project.description}
                    </p>
                  )}
                  ...
                  <div className="flex gap-4 mt-4">
                    {project.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={`text-xs ${t.accent} hover:opacity-80 transition`}
                      >
                        Live ↗
                      </a>
                    )}
                    {project.githubUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={`text-xs ${t.textSubtle} hover:${t.text} transition`}
                      >
                        GitHub ↗
                      </a>
                    )}
